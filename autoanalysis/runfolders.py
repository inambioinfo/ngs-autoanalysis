...
class RunFolders(object):
    ...
    """
    def __init__(self, basedir, destdir, one_run_folder=None, do_create_destdirs=True):
    ...
    def get_published_runs(self):
        # Analysis completed and publishing assigned - data may not be yet publicly available
        published_runs = []
        for run in self.completed_runs:
...
class RunDefinition(object):
    ...
    def create_destination_runfolder(self):
        if self.is_sequencing_completed() and self.destdir and self.do_create_destdir:
            return utils.locate_run_folder(self.run_folder_name, self.destdir)
        elif self.is_sequencing_completed() and self.destdir:
            return os.path.join(self.destdir, self.run_folder_name)
        return None
        
    def update_sequencing_status(self, _is_sequencing_complete, _dry_run=True):
...
class RunDefinitionTests(unittest.TestCase):
    ...
    def test_completed_run(self):
        self.assertTrue(self.run_def.is_sequencing_completed())
