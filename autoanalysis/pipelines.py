...
class PipelineDefinition(object):
    ...
    def __init__(self, run, pipeline_name, software_path=cfg['SOFT_PIPELINE_PATH'], cluster_host=None, use_limsdev=True, mode='local'):
        ...
        if self.pipeline_name == 'alignment':
            self.pipeline_setup_options = cfg['PIPELINES_SETUP_OPTIONS'][pipeline_name] % (socket.gethostname(), str(os.path.join(self.run.staging_run_folder, self.pipeline_name))[1:])
        else:
